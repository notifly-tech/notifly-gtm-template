...
# Notifly GTM Template llms.txt generator
# Usage:
...
#   EXCLUDE_DIRS    pruned dirs (default: node_modules build dist .git .idea .vscode .github .next)
...
path_is_target() {
  ...
  case "$base" in
    README.md|CHANGELOG.md|LICENSE|metadata.yaml) return 0 ;;
  esac
  # Check extension
  local ext="${rel##*.}"
  local match_ext=0
